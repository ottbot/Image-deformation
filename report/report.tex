...
deforming vector is defined directly on the image itself (\fig{inner_metric}),
as opposed to the \emph{outer metrics} used in LDDMM where the deforming vector
is applied to the entire field (\fig{outer_metric}). We use an inner metric to
...
%Unlike LDDMM, the inner metric used
%requires the curve be differentiable, which may be a limitation in some
%applications.

% Inner metrics on planar curves were introduced and studied previously in [11,
% 12, 16]. Recently several examples of inner metrics were studied on surfaces and
% higher dimensional hypersurfaces in Euclidean space in [1, 2]. The numerical
% implementation of inner metrics for matching differs from LDDMM, because the
% metric on the tangent space at each surface depends nonlinearly on the
% surface. This means the metric will change adaptively as one moves around in
% shape space. This adaptive property of the inner metrics is in marked contrast
% to LD- DMM, where the outer metric is defined on the diffeomorphisms and
% projected down to the shapes, so it doesn’t depend on the particular shape.
...
Relaxing the constraint \eq{dqdt} and introducing the Lagrange multiplier
...
\end{equation}
We first find an equation for  $\vv{J}{\vect q}$ using the definition of the functional derivative \eq{lim_dS}
\begin{equation}
  \label{eq:lim_dJdq}
...
$S[\vect u]$, the integral with respect to $t$ in \eq{S} is replaced with a
...
q\big|_{t=1}$ is known from \eq{qh_t1}, written in discretised form using a
...
\begin{equation}
  ...
  \angles{\hvect w, \hvect q^N} = \frac{1}{\sigma^2}\angles{\hvect w, \vect q^N
    - \vect q^B}.
\end{equation}
Discretisation of \eq{dqhdt} is achieved
...
\item Find $\hvect q|_{t=1}$ from \eq{qht1_disc}.
...
\begin{figure}[H]
  ...
  \caption[Implementation convergence test results]{Convergence test using first and second order approximation of
    \eq{lim_dS}. Both first and second order errors converge to the same error
    before machine precision becomes an issue. }
  \label{fig:convergence}
\end{figure}

On the log-log plot in \fig{convergence}, the errors $E_1$ and $E_2$ linearly approach zero as $\eps \rightarrow 0$
...
\fig{vectors} shows the progressive deformation of a circular template
...
and curve evolution in \fig{reparams} would be the same, with equal values of
$S[\vect u]$.
...
(\fig{hand_star}), and the shape database (\fig{shapedb}) are given as PNG
...
of $11$ the serif fonts were successfully were classified as such. \fig{sabon}
suggests this is likely again due to the parameterisation dependence of the
penalty term in \eq{S}. Here, it appears by visual inspection Times New Roman
...
3D surfaces. The metric \eq{metric} needs be rewritten for surfaces in
$\mathbb{R}^3$, with a suitable functional already available in
\cite{bauer2011new}. After reformulating and discretising \eq{S} along with its
